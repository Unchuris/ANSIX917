...
bool verifyKey(BinStr key) {
	...
	for (int i = 0; i < DES_KEY_SIZE; i += BITS_PER_BYTE) {
		BinStr section = subString(key, i, i + BITS_PER_BYTE - 1);
		if (parity(section) == 0) {
			return 0;
		}
	}
...
BinStr DESencrypt(BinStr block, BinStr *roundKeys) {
	...
	BinStr L = subString(new, 0, (DES_BLOCK_SIZE / 2) - 1);
	BinStr R = subString(new, DES_BLOCK_SIZE / 2, DES_BLOCK_SIZE - 1);
...
BinStr DESdecrypt(BinStr block, BinStr *roundKeys) {
	...
	BinStr new = iPermutation(block);
	BinStr R = subString(new, 0, (DES_BLOCK_SIZE / 2) - 1);
	BinStr L = subString(new, DES_BLOCK_SIZE / 2, DES_BLOCK_SIZE - 1);
